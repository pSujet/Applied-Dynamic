function X_dot = eomN_3DOF_planar_robot_manipulator(t, X, params)
...
Io1         = I_lcm1+m1*lcm1^2;     % Moment of inertia of link 1 around origin [kg.m^2]

%% Dynamics

% B(theta)*theta_ddot + C(theta,theta_dot)*theta_dot + Fv*theta_dot + Fs*sign(theta_dot) + g(theta) = tau - J'(theta)*he

b11 = Io1+m2*(l1^2+lcm2^2+2*l1*l2*cos(theta2))+Icm2+kr1^2*I_M1;
b12 = m2*(lcm2^2+l1*lcm2*cos(theta2))+Icm2+I_M2*kr2;
b13 = 0;
b21 = b12;
b22 = m2*lcm2^2+I_cm2+I_M2*kr2^2;
b23 = 0;
b31 = 0;
b32 = 0;
b33 = Io1*cos(theta1)^2+m2*(lcm2*cos(theta1+theta2)+l1*cos(theta1))^2 ...
      +Icm2*cos(theta1+theta2)^2+Icm3+M2*l1^2*cos(theta1)^2+kr3^2*I_M3;
  
c11 = 0;
c12 = -m2*l1*lcm2*theta2_dot*sin(theta2)*(2*theta1_dot+theta2_dot);
c13 = (m2*(lcm2*sin(theta1+theta2)+l1*sin(theta1))*(lcm2*cos(theta1+theta2)+l1*cos(theta1)) ...
       +Icm2*cos(theta1+theta2)*sin(theta1+theta2)+cos(theta1)*sin(theta1)*(Io1+M2*l1^2))*theta3_dot;
c21 = m2*l1*lcm2*sin(theta2)*theta1_dot;
c22 = 0;
c23 = (m2*lcm2*sin(theta1+theta2)*(lcm2*cos(theta1+theta2)+l1*cos(theta1))+Icm2*cos(theta1+theta2) ...
       *sin*(theta1+theta2))*theta3_dot;
c31 = (-2*m2*(lcm^2*sin(theta1+theta2)*cos(theta1+theta2)+l1*lcm2*cos(theta1)*sin(theta1+theta2) ...
       +l1*sin(theta1)*(lcm2*cos(theta1+theta2)+l1*cos(theta1))) ...
       -2*Io1*cos(theta1)*sin(theta1)-2*Ic2*cos(theta1+theta2)-2*M2*l1^2*cos(theta1))*theta3_dot;
c32 = (-2*m2*(lcm^2*sin(theta1+theta2)*cos(theta1+theta2)+l1*lcm2*cos(theta1)*sin(theta1+theta2)) ...
      -2*Ic2*cos(theta1+theta2))*theta3_dot;
c33 = 0;

g11 = (m1*lcm1+(m2+M1+M2)*l1)*g*cos(theta1)+(m2*l2+M2*l2)*g*cos(theta1+theta2);
g21 = (m2*lcm2+M2*l2)*g*cos(theta1+theta2);
g31 = 0;
